...
import {AiItemInputSchema} from './src/ai-validation.ts';
import {
    estimatePrice,
    generateDescription,
...
fastify.post('/ai/generate-description', async (request, reply) => {
    try {
        const payload = AiItemInputSchema.parse(request.body);
        return await generateDescription(payload);
    } catch (error) {
        if (error instanceof ZodError) {
            reply.status(400).send(treeifyError(error));
            return;
        }
...
fastify.post('/ai/estimate-price', async (request, reply) => {
    try {
        const payload = AiItemInputSchema.parse(request.body);
        return await estimatePrice(payload);
    } catch (error) {
        if (error instanceof ZodError) {
            reply.status(400).send(treeifyError(error));
            return;
